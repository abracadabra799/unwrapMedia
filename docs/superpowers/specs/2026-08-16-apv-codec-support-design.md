...
- One Detail Properties section, **"APV Frame Header"**, rendered when a frame is selected on an APV stream: curated `frame_header()` fields (see Data model below), resolved lazily per selected frame — no stateful whole-stream pass needed, since every frame's header is self-contained.
...
**Verified against RFC 9924 (the official APV RFC) and the `AcademySoftwareFoundation/openapv` reference-implementation source — not against a real generated APV file**, since this dev machine's ffmpeg has APV decode support but no encoder (`ffmpeg -h encoder=apv` reports "Codec 'apv' is known to FFmpeg, but no encoders for it are available... needs libopenapv"). This is a deliberate, disclosed exception to this project's usual real-file-verification precedent (H.264/HEVC/AV1/Quality Compare) — the implementation plan **must** open by acquiring a real captured/reference APV sample (e.g. from `openapv`'s own test-vector assets, or a real camera/software export if available) and byte-verifying the framing below before writing Kotlin.
...
**ISOBMFF container mapping** (from `openapv`'s `readme/apv_isobmff.md`): sample entry fourcc is **`apv1`** (`APV1SampleEntry`); the config box is **`apvC`** (`APVCodecConfigurationBox`, containing an `APVDecoderConfigurationRecord` with per-configuration-entry `profile_idc`/`level_idc`/`band_idc`/`frame_width`/`frame_height`/`chroma_format_idc`/`bit_depth_minus8` — the same `frame_info()` fields as the bitstream itself, present redundantly for fast container-level inspection). Each MP4 sample holds one access unit, described as "the length field of four bytes preceding the access unit data." **Open verification item:** whether that leading 4-byte MP4-sample length field is *in place of* the bitstream's own `access_unit(au_size)` leading `signature f(32)` field, or *in addition to* it (i.e., does an MP4 sample's bytes begin directly with `[pbu_size][pbu()]...`, or with `['aPv1' signature][pbu_size][pbu()]...`?) — the fetched documentation text doesn't resolve this unambiguously. The implementation plan must confirm this against a real sample's raw bytes before writing the container-extraction code; both cases are simple length-driven parsing either way; only the leading-offset (0 vs. 4 bytes) differs.
...
- No real captured APV file is available in this environment (see Technical Foundation) — the implementation plan's first task must acquire one (or, failing that, hand-construct a byte-exact synthetic fixture directly from the RFC 9924 syntax tables above, clearly documented as spec-derived rather than tool-verified, with a follow-up noted to replace it with real-file verification once a sample is available).
- `ApvPbu.kt`/`ApvFrameHeader.kt` unit tests follow this codebase's established convention: real (or, per the caveat above, spec-derived synthetic) byte fixtures, one assertion per curated field, plus null-on-malformed-input cases (truncated PBU, unrecognized `pbu_type`, out-of-range `chroma_format_idc`).
- `ApvCBoxDecoder` and the container-extraction functions get synthetic byte-fixture tests (matching the `avcC`/`hvcC`/`av1C` extraction test convention) for the box-layout logic itself, which doesn't depend on real encoded frame content to verify.
- Manual verification (once a real file is available): open it, select frames, confirm shown fields are internally consistent (e.g. `frame_width`/`frame_height` matching the container's own track dimensions) and match `ffmpeg -v verbose`'s own stream-info output for the same file as an independent cross-check (ffmpeg's APV *decoder* exists on this dev machine even without an encoder).
